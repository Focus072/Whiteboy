...
import { getOrder, logStakeCall, shipOrder } from '@/lib/admin-api';
...
export default function OrderDetailPage() {
  ...
  const handleShip = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setActionError(
          (response.error?.message || 'Failed to ship order') + (reasons.length > 0 ? `: ${reasons.join(', ')}` : '')
        );
      }
    } catch {
      setActionError('Network error. Please try again.');
  ...
  const shipDisabledReason = !order
    ? 'Loading...'
    : order.status !== 'PAID'
      ...
      : order.complianceSnapshot?.finalDecision !== 'ALLOW'
        ...
        : order.shippingAddress?.line1?.toLowerCase().includes('po box')
          ...
          : order.payments?.[0]?.status !== 'AUTHORIZED'
            ? `Payment status is ${order.payments?.[0]?.status}, must be AUTHORIZED`
            : order.complianceSnapshot?.stakeCallRequired && !order.stakeCalls?.length
              ? 'STAKE Act call required'
              : '';
  ...
  if (error || !order) {
    return (
      <div className="px-4 py-6">
        ...
        <Link href="/admin/dashboard/orders" className="text-indigo-600 hover:text-indigo-900">
          ← Back to Orders
        </Link>
      </div>
    );
              ...
              {order.items.map((item) => (
                <div key={item.id} className="border-b border-gray-200 pb-3 last:border-0">
                  ...
                  <div className="text-sm text-gray-600">
                    Weight: {item.product.netWeightGrams * item.quantity}g
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span
                    className={
                      order.ageVerification.status === 'PASS'
                        ? 'text-green-600 font-semibold'
                        : 'text-red-600 font-semibold'
                    }
                  >
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span
                    className={
                      order.payments[0].status === 'AUTHORIZED' || order.payments[0].status === 'CAPTURED'
                        ? 'text-green-600 font-semibold'
                        : 'text-red-600 font-semibold'
                    }
                  >
          ...
          {order.status !== 'SHIPPED' && (
            <div className="bg-white shadow rounded-lg p-6">
              ...
              {!canShip && shipDisabledReason && <p className="mt-2 text-xs text-gray-500">{shipDisabledReason}</p>}
            </div>
          )}
        </div>
      </div>
